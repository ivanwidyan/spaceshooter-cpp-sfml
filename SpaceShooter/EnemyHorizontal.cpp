...
EnemyDiagonal::EnemyDiagonal(){
	if (!enemyRed.loadFromFile("Sprite/Enemy2.png")) {}
	enemyRed.setSmooth(true);
	enemySprite.setTexture(enemyRed);
	moveLeft = rand() % 2;
	if (moveLeft) {	enemySprite.setRotation(45); }
	else { enemySprite.setRotation(-45); }
}
